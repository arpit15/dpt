...
std::unique_ptr<const Scene> ParseScene(pugi::xml_node node);
...
std::shared_ptr<const Shape> ParseShape(pugi::xml_node node,
    ...
    if (type == "serialized") {
        ...
        shape = std::make_shared<TriangleMesh>(
            bsdf, LoadSerialized(filename, shapeIndex, toWorld[0], toWorld[1], isMoving));
    } else if (type == "obj") {
        std::string filename;
        Matrix4x4 toWorld[2];
        toWorld[0] = toWorld[1] = Matrix4x4::Identity();
        ...
        shape = std::make_shared<TriangleMesh>(
            bsdf, ParseObj(filename, toWorld[0], toWorld[1], isMoving));
    }
...
std::shared_ptr<const DptOptions> ParseDptOptions(pugi::xml_node node) {
    ...
    for (auto child : node.children()) {
        ...
        if (name == "integrator") {
            ...
            dptOptions->largeStepProb = std::stof(child.attribute("value").value());
        ...
        } else if (name == "lensperturbstddev") {
            dptOptions->lensPerturbStdDev = std::stof(child.attribute("value").value());
        } else if (name == "roughnessthreshold") {
            dptOptions->roughnessThreshold = std::stof(child.attribute("value").value());
        } else if (name == "lensperturbprob") {
            dptOptions->lensPerturbProb = std::stof(child.attribute("value").value());
...
std::unique_ptr<const Scene> ParseScene(pugi::xml_node node) {
    std::shared_ptr<const DptOptions> options = std::make_shared<DptOptions>();
    ...
    return std::unique_ptr<const Scene>(
